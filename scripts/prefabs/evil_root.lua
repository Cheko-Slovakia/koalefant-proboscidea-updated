...
local function onpickedfn(inst, picker)	
	if picker and picker.components.sanity then
		...
		GetWorld().components.special_flower_spawner.evil_root = 0
	end
	if picker.components.combat then
        picker.components.combat:GetAttacked(inst, TUNING.ROOT_DAMAGE)
...
local function makebarrenfn(inst)
	inst.MiniMapEntity:SetIcon("evil_root_dead.tex")
	inst.AnimState:PlayAnimation("dead")
end

local function makefullfn(inst)
	inst.MiniMapEntity:SetIcon("evil_root_blooming.tex")
...
local function fn()
    ...
    inst.Light:SetColour(239/255, 194/255, 194/25)
    inst.Light:SetIntensity(0)
    inst.Light:Enable(false)
    ...
    inst.AnimState:SetBank("evil_root")
    inst.AnimState:SetBuild("evil_root")
    inst.AnimState:PlayAnimation("blooming")
    inst.AnimState:SetRayTestOnBB(true);
	
	inst._fadeval = net_float(inst.GUID, "evil_root._fadeval")
    inst._faderate = net_smallbyte(inst.GUID, "evil_root._faderate", "onfaderatedirty")
    inst._fadetask = nil
	
	inst:AddTag("evil_root")
...
return Prefab( "forest/objects/evil_root", fn, assets, prefabs) 
